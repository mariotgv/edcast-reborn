...
void CFlexMeters::drawmeter_horizontal(int value, int * buf, int ymulti, int xp, int yp, int x_size, int y_size, RGBQUAD * colourArray, eMeterDirection direction, int peak)
{
    int x = 0;
    int y = 0;
	int yofs = 0;
    ...
    {
        threshRMS = xp + (((x_size) * (MAX_VALUE-value)) / MAX_VALUE);
        threshPeak = xp + (((x_size) * (MAX_VALUE-peak)) / MAX_VALUE);
		if(threshRMS > threshPeak) thresh = threshPeak; else thresh = threshRMS;
    }
    else
    {
        threshRMS = xp + (((x_size) * (value)) / MAX_VALUE);
        threshPeak = xp + (((x_size) * (peak)) / MAX_VALUE);
		if(threshRMS < threshPeak) thresh = threshPeak; else thresh = threshRMS;
    }

    for (x = xp;x < xp + x_size;x++)
    {
		int cindex = (x_size-1)-(x-xp);
		localcolour = *((int *) &colourArray[cindex]);
        if ((x < threshRMS && direction == eMeterDirection_Normal) || (x >= threshRMS && direction == eMeterDirection_Backwards))
        {
			;//localcolour = *((int *) &colourArray[cindex]);
        }
        else
        {
	        if (peak > 0 && value > 0 && x > threshPeak - 1 && x < threshPeak + 1)
			{
				;
			}
			else
...
void CFlexMeters::RenderMeters(HDC hDC)
{
	...
	if(fmis.horizontal_meters)
	{
		for(a=0;a<fmis.meter_count;a++)
		{
			...
			drawmeter_horizontal( pMeter->value,intbuffer,fmis.max_x,pMeter->length_offset,pMeter->position,pMeter->length,pMeter->width,pMeter->colourArray,pMeter->direction, pMeter->peak);
		}
	}
	else
	{
